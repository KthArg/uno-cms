...
| **Acceso por proveedor externo** | Google es **opcional y aditivo** (ADR-900): sin sus dos variables el proveedor no entra en la configuración. Con él, tres puertas —`email_verified`, que el correo corresponda a una cuenta **ya existente** y que esté activa— y **nunca crea cuentas ni decide roles**; la identidad de la sesión se toma de `users`, no del perfil | T-233-2 (sin variables no hay proveedor), T-233-5 a T-233-7 (las tres puertas), T-233-9 y T-233-10 (la identidad sale de la fila), T-233-12 (un rechazo no crea ninguna cuenta), T-233-14 (ADR-301 alcanza también a esta puerta) |
