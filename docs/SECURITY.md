...
| **Acceso por proveedor externo** | **Hoy no hay ninguno: el acceso con Google se entregó apagado** (`ACCESO_CON_GOOGLE_HABILITADO = false`, #240), y el interruptor gana a `AUTH_GOOGLE_ID`/`AUTH_GOOGLE_SECRET`, así que la superficie no está viva. Escrito y probado para cuando se encienda (ADR-900): tres puertas —`email_verified`, que el correo corresponda a una cuenta **ya existente** y que esté activa— y **nunca crea cuentas ni decide roles**; la identidad de la sesión se toma de `users`, no del perfil | T-233-19 (con las dos variables puestas, sigue sin haber proveedor), T-233-2 (sin variables tampoco), T-233-5 a T-233-7 (las tres puertas), T-233-9 y T-233-10 (la identidad sale de la fila), T-233-12 (un rechazo no crea ninguna cuenta), T-233-14 (ADR-301 alcanza también a esa puerta) |
